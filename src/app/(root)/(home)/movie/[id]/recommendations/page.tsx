...
interface ParamsRecommendations {
  params: {
    id: MovieCardInterface["id"];
  };
}

async function getAllRecommendations(id: MovieCardInterface["id"]) {
...
const page = async ({ params }: ParamsRecommendations) => {
  const { id } = params;
    ...
    <main className="mt-5 flex flex-col mb-6">
      <div className="w-[1200px] max-w-full px-4 mx-auto">
        ...
        <div className="grid grid-cols-4 mt-4 gap-4">
          {movieRecommendations?.results?.map((movie: MovieCardInterface) => (
